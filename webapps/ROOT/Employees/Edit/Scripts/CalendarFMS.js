...
var img_close;
img_del=new Image();
img_del.src="/Images/Calendar/btn_del_small.gif";
img_close=new Image();
img_close.src="/Images/Calendar/btn_close_small.gif";
...
img_Date_UP.src="/Images/btnbuscar.gif";
...
function showCalendarFMS(frmName, dteBox,opener, mya, mym, hideDrops, MnDt, MnMo, MnYr, MxDt, MxMo, MxYr,runFuncs) {
    if( (mya!=null) && (mym!=null) ){
        minYearList=todayDate.getFullYear()-mya;
        maxYearList=todayDate.getFullYear()-mym;
    }else{
        minYearList=todayDate.getFullYear()-10;
        maxYearList=todayDate.getFullYear()+10;
    }
    ...
    if (IsCalendarVisible) {
        ...
        curDateBox = dteBox;
        if ( ppcIE ) {
			ppcX = getOffsetLeft(document.getElementById(opener));
			ppcY = getOffsetTop(document.getElementById(opener))+25;
			
			ppcYW = f_get_window_height();
			if (ppcY > ppcYW - 150) ppcY = ppcYW - 150;
        }
        else if (ppcNN){
            ppcX = document.getElementById(opener).x; 
            ppcY = document.getElementById(opener).y + document.getElementById(opener).height;
            
            ppcYW = f_get_window_height();
			if (ppcY > ppcYW - 150) ppcY = ppcYW - 150;
        }

        domlay('popupcalendar',1,ppcX,ppcY,CalendarFMS(todayDate.getMonth(),todayDate.getFullYear()));       
...
function CalendarFMS(whatMonth,whatYear) {
    var output = '';
    var datecolwidth;
    var startMonth;
    var startYear;
    ...
    output += '<td class="cal-HeadCell" align="center" width="100%"><a href="javascript:clearDay();"><img name="calbtn1" src="/Images/Calendar/btn_del_small.gif" border="0" width="12" height="10"></a>&nbsp;&nbsp;<a href="javascript:scrollMonth(-1);" class="cal-DayLink">&lt;</a>&nbsp;<SELECT class="cal-TextBox" NAME="cboMonth" onChange="changeMonth();">';
    ...
    output += '<\/SELECT>&nbsp;<a href="javascript:scrollMonth(1);" class="cal-DayLink">&gt;</a>&nbsp;&nbsp;<a href="javascript:hideCalendar();"><img name="calbtn2" src="/Images/Calendar/btn_close_small.gif" border="0" width="12" height="10"></a><\/td><\/tr><tr><td width="100%" align="center">';
...
function clearDay() {
	curControl = document.getElementById(curDateBox);
	curControl.value = '';
    hideCalendar();
    if (FuncsToRun!=null)
        eval(FuncsToRun); 
...
function changeDay(whatDay) {
    ...
    curControl = document.getElementById(curDateBox);
    //alert(curDate.getFullYear()+''+padout(curDate.getMonth()+1)+''+padout(curDate.getDate()));
    curControl.value = padout(curDate.getDate())+'/'+padout(curDate.getMonth()+1)+'/'+curDate.getFullYear();
...
function changeMonth() {
    ...
    if (ppcIE) {        
        ...
        domlay('popupcalendar',1,ppcX,ppcY,CalendarFMS(curDate.getMonth(),curDate.getFullYear()));
    }
    else if (ppcNN) {
