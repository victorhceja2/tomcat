...
<%
    try{
		...
		lbIsFranq = Boolean.valueOf(isFranq());

		if(lbIsFranq){
			msLocalStore   = msType.equals("input")?getFLocalStore(msTransferId):getFRemoteStore(msTransferId);
			msRemoteStore  = msType.equals("input")?getFRemoteStore(msTransferId):getFLocalStore(msTransferId);
		}else{
...
<body bgcolor="white"
	...
	<table width="99%" border="0" align="center" cellspacing="6">
		<tr>
			<td width="20%" class="descriptionTabla" nowrap>Centro de
				contribuci&oacute;n origen:</td>
			<td width="80%" class="descriptionTabla"><b><%=msRemoteStore%></b>
			</td>
		</tr>
		<tr>
			<td class="descriptionTabla" nowrap>Centro de
				contribuci&oacute;n destino:</td>
			<td class="descriptionTabla"><b><%=msLocalStore%></b></td>
		</tr>
		<tr>
			<td class="descriptionTabla" nowrap>Fecha de realizaci&oacute;n:
			</td>
			<td class="descriptionTabla"><b><%=msTransferDate%></b></td>
		</tr>
	</table>
	<table width="99%" border="0" align="center" cellspacing="6">
		<tr>
			<td><br>
				<div id="goDataGrid"></div></td>
		</tr>
	</table>
...
<%!String getQueryReport(String psTransferId) {
		if (Boolean.valueOf(isFranq())) {
			loadFCurrentExistence(true);
			return getFTransferDetailQuery(false, psTransferId); //false: NO de la tabla de paso
		} else {
			loadCurrentExistence(true);
