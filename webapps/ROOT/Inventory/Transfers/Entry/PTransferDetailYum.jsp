...
<%
	...
	if(lsNStore == null){
		try{
			lsNStore=request.getParameter("hidRemoteStore");
		}catch(Exception e){
		...
		logApps.writeInfo("\n" + new Date() + ": Se solicitará producto final al CC " + lsNStore);
	}
%>
<html>
<head>
...
<script>
	...
	function cargaContenido(lsTable, clss, mult) {
		...
		var ajax = nuevoAjax();
		ajax.open("GET", "PChooseClassYum.jsp?class=" + clss + "&table="
				+ lsTable + "&mult=" + mult, true);
		ajax.onreadystatechange = function() {
			if (ajax.readyState == 1) {
				selectDestino.innerHTML = "<p class=descriptionTabla style=\"font-size: 18px;\">Cargando "
						+ lsTable + "...</p>";
			}
...
<%=lsNStore%>
	...
	function validNumTopps(noTops) {
		var clss = document.getElementById("clases").value;
		if (noTops == '1') {
			var prod = document.getElementById("prod").value;
			var selTopp = document.getElementById("topp");
			if (selTopp.type == "select-multiple") {
				var topp = "";
				for (var elem = 0; elem < selTopp.length; elem++) {
					if (selTopp.options[elem].selected) {
						topp += selTopp.options[elem].value + ",";
				...
				data.push(prod);
				data.push(topp);
				jsrsExecute("RemoteProductMethods.jsp", muestraAlertaTop,
						"validaNumToppings", data);
			}
		} else {
			var prod = document.getElementById("prod2").value;
			var selTopp = document.getElementById("topps2");
			if (selTopp.type == "select-multiple") {
				var topp = "";
				for (var elem = 0; elem < selTopp.length; elem++) {
	...
	function muestraAlertaTop(lsResult) {
		if (lsResult != "OK") {
			alert("Solo se permite seleccionar " + lsResult + " toppings");
			//focusElement('topp');
		}
	}
	...
	function validateCredentials(rsUserPwd) {
		if (rsUserPwd == "FALSE" || rsUserPwd == "ERROR") {
			...
			document.frmGrid.hidNeighborStore.value = parent
					.getIdNeighborStore();
			document.frmGrid.hidUser = parent.getUser();
						...
						<select name="clases" id="clases"
							...
							<%
								...
								for (String lrClases : laClases) {
									//logApps.writeInfo("lrClases: " + lrClases);
								...
								<td>
									<div id="btnActualizar" style='dysplay: none;'>
										<input type="button" value="Actualizar" id="btnUpdate"
											name="btnUpdate" onclick="submitUpdate()">
									</div>
								</td>
