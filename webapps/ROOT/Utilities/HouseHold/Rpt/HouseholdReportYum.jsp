...
<%
    try
    {
		...
		msPreviousYear	= getPreviousYear(msYear);
		...
		if(msReport.equals("2"))//Se tiene que escoger una semana
		{
			...
			//TODO: considerar semanas cerradas.
            if(begindate.compareTo(currentdate) >= 0) //No fechas futuras
			{	
				msMessage = "No se pueden obtener reportes para semanas que no han cerrado.";
			}
			else
			{
				reportOk  = reportOk(msPreviousYear, msYear, msPeriod, msWeekId);

				if(reportOk)
	    			msDataset = getDataset(msPreviousYear, msYear, msPeriod, msWeekId);
                else
				{
...
<html>
		...
		<script>
			...
			function doAction()
			{
				...
				{	
					...
					addHidden(document.mainform, 'hidTitle', 'Reporte de Transacciones por AGEB');
	...
	<body onLoad="doAction()" style="margin-left: 0px; margin-right: 0px">
        ...
        <form name="mainform" action="HouseholdReportFrm.jsp">
