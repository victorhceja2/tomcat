...
cd $WEBAPPS
	...
	then
        ...
        WAY_ORDER=`psql -d dbeyum -U postgres -c "select count(*) from pg_catalog.pg_tables WHERE tablename='op_grl_way_order' " -t -A`

		if [ $WAY_ORDER -eq 0 ]
		then
		    phzap psql -U postgres dbeyum < ROOT/SQL/$SQLCHANGES
		else
			echo "Tabla op_grl_way_order existe ... no se crea."
		fi
	fi
