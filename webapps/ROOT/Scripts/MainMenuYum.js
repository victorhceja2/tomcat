/*
##########################################################################################################
# Nombre Archivo  : MainMenuYum.js
# Compañia        : Yum Brands Intl
# Autor           : JPG
# Objetivo        : Funciones para la creación de menus
...
msAgent=navigator.userAgent.toLowerCase();
mbIsMajor = parseInt(navigator.appVersion);
mbIsMenor = parseFloat(navigator.appVersion);
mbIsNN = (window.innerHeight)?true:false;
mbIsIE = ((msAgent.indexOf("msie") != -1) && (msAgent.indexOf("opera") == -1));
mbIsIE5 = (mbIsIE && (mbIsMajor == 4) && (msAgent.indexOf("msie 5.0")!=-1) );
mbIsIE5_5  = (mbIsIE && (mbIsMajor == 4) && (msAgent.indexOf("msie 5.5") !=-1));
...
// Modificación para ocultar todo un div
function fixForm(divname,show) {
    if (!mbIsIE5 && !mbIsIE5_5) return;
    var lsShowHide;
    var loDiv = _d.getElementById(divname);

    if (show) lsShowHide = 'visible'; else lsShowHide = 'hidden';
    if (loDiv) loDiv.style.visibility = lsShowHide;
}
...
function resetShM(){ShM=0;if(ParentMenu)ParentMenu.resetShM()}
