    ...
    function initDataGrid(isReport){
		...
		if(gaDataset.length > 0){
			...
			headers  = new Array(
				...
				// 2: Concentracion
				{text:'Clientes',width:'50%',align:'center',sort:'numeric'});
	...
	for(var rowId=0; rowId<data.length; rowId++){
		for(var columnId=0; columnId< data[rowId].length;columnId++){
			if(in_array(columnId, cols)){
				...
				var frecu = data[rowId][2];
				if(newValue.length == 0 || newValue == '')
					newValue = '&nbsp;';
				else
					newValue = '<a href=javascript:openPopUp(\"CustomDetailYumA.jsp?hidFamily=' + data[rowId][4] + '&hidFrecu=' + frecu + '&hidDate=' + data[rowId][3] + '&hidAgeb=' + data[rowId][0] + '\");>' + datagrid[rowId][0] + '</a>';
...
function openPopUp(url){
	window.open(url, 'Detalle de Concentracion por Calles','width=700px, height=500px, left=40, top=40, menubar=no, resizable=yes, scrollbars=YES');
}
