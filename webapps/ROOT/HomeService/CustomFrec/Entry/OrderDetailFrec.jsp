<%--
##########################################################################################################
# Nombre Archivo  : OrderDetailFrec.jsp
# Compania        : Yum Brands Intl
# Autor           : GAR
# Objetivo        : Detalle Orden Clinetes
# Fecha Creacion  : 19/Mar/2008
...
<FRAMESET rows="99%, 1%" border="0">
	<FRAME src="OrderDetailFrmF.jsp?target=Preview&noteId=<%= tel %>&nombre=<%= nam %>&fecha=<%= fec %>" name="preview" frameborder="0">
    <FRAME src="OrderDetailFrmF.jsp?target=Printer&noteId=<%= tel %>&nombre=<%= nam %>&fecha=<%= fec %>" name="printer" frameborder="0">
</FRAMESET>
...
<%!
    ...
    String getQueryReport(){
   		String lsQuery = "select I.qty_sold, CC.cdesc,SB.bdesc," +
						...
						//System.out.println("Items Compra: " + lsQuery);
	...
	String getQueryReportU(){
		String lsQueryU = "select case WHEN C.promo_allowance <> '0.00' THEN " +
						  " (select coupon_code from gc_items g inner join gc_coupons c on g.store_id = c.store_id and g.date_id = c.date_id" + 
						  " and g.gc_sequence = c.gc_sequence and g.it_seq = c.it_seq and g.date_id = D.date_id and g.gc_sequence = D.gc_sequence" + 
						  " inner join gc_coupon_codes cc on c.coupon_id = cc.coupon_id limit 1)"+ 
						...
						//System.out.println("Fechas compra: " + lsQueryU);
