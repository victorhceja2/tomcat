...
<%--
##########################################################################################################
# Nombre Archivo  : IdealUseReport14Yum.jsp
# Compania        : Yum Brands Intl
# Autor           : Sergio Cuellar
# Objetivo        : uso ideal de 14 dias atras, pase lo que pase
# Fecha Creacion  : 19/febrero/2013
# Inc/requires    :
# Modificaciones  :
##########################################################################################################
--%>
...
<%
    try
    {
		...
		{
		        reportOk  = true;
                        biscuitsDataset    = getDataset(msWeek, msYear, msPeriod, msWeekId, msDay, "Biscuits.conf", 14);
                        congeladosDataset  = getDataset(msWeek, msYear, msPeriod, msWeekId, msDay, "Congelados.conf", 14);
                        ensaladaDataset    = getDataset(msWeek, msYear, msPeriod, msWeekId, msDay, "Ensalada.conf", 14);
                        homeDataset        = getDataset(msWeek, msYear, msPeriod, msWeekId, msDay, "Home_Delivery.conf", 14);
                        marinadoDataset    = getDataset(msWeek, msYear, msPeriod, msWeekId, msDay, "Marinado.conf", 14);
                        pureDataset        = getDataset(msWeek, msYear, msPeriod, msWeekId, msDay, "Pure.conf", 14);
                        purensaladaDataset = getDataset(msWeek, msYear, msPeriod, msWeekId, msDay, "Pure_Ensalada.conf", 14);
                        sandwichDataset    = getDataset(msWeek, msYear, msPeriod, msWeekId, msDay, "Sandwich.conf", 14);
                        servicioDataset    = getDataset(msWeek, msYear, msPeriod, msWeekId, msDay, "Servicio.conf", 14);
                        trasempaqueDataset = getDataset(msWeek, msYear, msPeriod, msWeekId, msDay, "Trasempaque.conf", 14);
                        PostresDataset     = getDataset(msWeek, msYear, msPeriod, msWeekId, msDay, "Postres.conf", 14);
		}
		else
		{
...
<html>
		...
		<script>
			...
			function doAction()
			{
				...
				{	
					...
					addHidden(document.mainform, 'hidTitle', 'Uso Real Diario');
	...
	<body onLoad="doAction()" style="margin-left: 0px; margin-right: 0px">
        ...
        <form name="mainform" action="IdealUseReport14Frm.jsp">
