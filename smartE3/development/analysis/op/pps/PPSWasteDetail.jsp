<%-- 
    Document   : PPSWasteDetail
    Created on : 26/04/2016, 04:58:36 PM
    Author     : DAB1379
--%>
...
<html>
    ...
    function getData(psProductId,psAmount,psHost){
          var lsQuery;
            ...
            lsQuery+="COALESCE(TRUNC(SUM(b.total)/"+psAmount+",1),0) total ";
            ...
            //lsQuery+="SELECT DATE_PART(''hour'',date_id),SUM(CAST(amount AS NUMERIC)) FROM op_pps_relay WHERE CAST(date_id AS DATE) IN (''<%//=msDate%>'') AND product_id = "+psProductId+" AND status = 20 GROUP BY 1 ') AS t( ";
          ...
          function(psData) {
            ...
            if(psData != ""){
                ...
                for(var li=0;li<lsData.split("_||_").length;li++){
                    ...
                    var lsWasted = lsRow.split("_|||_")[1];
                    loDateTicks.push([li,lsDate]);
                    laDataWasted.push([lsDate,lsWasted]);
                }
                ...
                if(psHost == "")
                    ...
                    $( "#divMain"+psProductId ).append( "<center><img width='200px' src='/smartE3/images/e3/ui/explorer/opt_btn.PNG'/><h3>No se encontr&oacute; sinformaci&oacute;n, verifica que el servidor del PPS est&eacute; ne l&iacute;nea y exista merma la fecha consultada</h3></center>");
            }
        },"text");
    }
    ...
    <body onload="">
        <center><div style="background-color:red;width:100%;color:white;border-radius: 5px"><table width="100%"><tr><td>Fecha(s):<br><%=msDate%></td><td width="80%"><h3><img width="50" src="/smartE3/images/e3/ui/explorer/mdw.png"><br>DETALLE DE MERMAS</h3></td><td id="tdStoreData">&nbsp;</td></tr></table></div></center>
